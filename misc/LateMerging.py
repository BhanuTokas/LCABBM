...
class ConceptDrifter:
    ...
    def perturbDDIM(self, img, z_pos, z_neg, delta=0.1):
        z_i = self.clip_interface.getImgEmbedding(img)
        z_i_mag = z_i.norm(dim=1)
        vector = z_pos-z_neg
        magnitude = (z_pos.norm(dim=1) + z_neg.norm(dim=1)) / 2
        ...
        orig_img = self.diff_interface.genImage(z_i)
        new_img = self.diff_interface.genImage(z_new)
        zT_0, timesteps = self.diff_interface.ddim_invert(orig_img, num_inference_steps=40)
        zT_1, timesteps = self.diff_interface.ddim_invert(new_img, num_inference_steps=40)
        zT_main, timesteps = self.diff_interface.ddim_invert(img, num_inference_steps=40)
        zT_new = zT_main + (zT_1 - zT_0)
        final_img = self.diff_interface.decode_latents_to_image(zT_new)
        if(probs[0]>probs[1]):
            return new_img, orig_img, final_img
        else:
            return orig_img, new_img, final_img
...
if __name__ == "__main__":
    ...
    zT, timesteps = c.diff_interface.ddim_invert(image, num_inference_steps=40)
    ...
    recon = c.diff_interface.reconstruct_from_zT(z_i, zT, num_inference_steps=40, guidance_scale=1.0, eta=0.0, generator=gen)
    recon.save("reconstructed_from_zT.png")

    img1, img2, img3 = c.perturbDDIM(image, z_pos, z_neg, delta=0.2)
