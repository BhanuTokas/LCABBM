...
class ConceptDrifter:
    ...
    def perturbDDIM(self, img, z_pos, z_neg, delta=0.1):
        ...
        orig_img = self.diff_interface.genImage(z_i)
        new_img = self.diff_interface.genImage(z_new)
        zT_0, timesteps = self.diff_interface.ddim_invert(
            orig_img, num_inference_steps=40
        )
        zT_1, timesteps = self.diff_interface.ddim_invert(
            new_img, num_inference_steps=40
        )
        zT_main, timesteps = self.diff_interface.ddim_invert(
            img, num_inference_steps=40
        )
...
if __name__ == "__main__":
    ...
    recon = c.diff_interface.reconstruct_from_zT(
        z_i, zT, num_inference_steps=40, guidance_scale=1.0, eta=0.0, generator=gen
    )
