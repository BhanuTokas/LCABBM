...
OUTPUT_PATH = f"outputs/ablation_man_{APPEND_DIR}.png"    # path for the saved output image
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            ...
            if path and os.path.exists(path):
                ...
                ax.text(0.5, 0.5, "missing", color="gray",
            ...
            if row == 0:
                ax.set_title(g_labels[col], color="white", fontsize=8,
                             pad=4, fontweight="bold")

            if col == 0:
                ax.set_ylabel(s_labels[row], color="white", fontsize=8,
                              rotation=0, labelpad=6, va="center", fontweight="bold")

    fig.suptitle("Image grid: s_factor (rows) × g_scale (columns)",
